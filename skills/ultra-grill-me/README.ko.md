...
### 세션 로그 파일
- 스킬이 발동되면 `logs/` 디렉토리 아래에 턴별 질문 내용, 사용자 답변, 갱신된 결정 사항이 마크다운 파일(`logs/session_YYYYMMDD_HHMMSS.md`)로 실시간 누적 저장됩니다. Socratic 검증의 발자취를 추후 히스토리로 조회하기 유用합니다.
